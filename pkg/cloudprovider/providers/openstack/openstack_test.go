...
func TestLoadBalancer(t *testing.T) {
	...
	status, err := lb.EnsureLoadBalancer(env.UUID, "RegionOne", nil, ports, hosts, api.ServiceAffinityClientIP)
	t.Logf("%s", status)
	if err != nil {
		t.Fatalf("%s", err)
	}
	...
	status, exists, err = lb.GetLoadBalancer(env.UUID, "RegionOne")
	if err != nil {
		t.Fatalf("%s", err)
	}
	t.Logf("%s", status)

	err = lb.EnsureLoadBalancerDeleted(env.UUID, "RegionOne")
	if err != nil {
		t.Fatalf("%s", err)
	}
}
...
func teardown() {
	...
	err = subnets.Delete(env.Openstack.network, subnet.ID).ExtractErr()
	if err != nil {
		log.Printf("Subnet %s not deleted: %s", subnet.ID, err)
	}
	net := env.Network
