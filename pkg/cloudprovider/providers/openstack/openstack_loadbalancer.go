...
func (os *OpenStack) GetLoadBalancer(name, region string) (*api.LoadBalancerStatus, bool, error) {
...
func (os *OpenStack) EnsureLoadBalancer(name, region string, loadBalancerIP net.IP, ports []*api.ServicePort, hosts []string, affinity api.ServiceAffinity) (*api.LoadBalancerStatus, error) {
	...
	_, exists, err := os.GetLoadBalancer(name, region)
	if err != nil {
	...
	if exists {
		...
		err := os.EnsureLoadBalancerDeleted(name, region)
		if err != nil {
...
func (os *OpenStack) UpdateLoadBalancer(name, region string, hosts []string) error {
...
func (os *OpenStack) EnsureLoadBalancerDeleted(name, region string) error {
